...
var TableWidget = PosBaseWidget.extend({
    ...
    renderElement: function(){
        var self = this;
	//console.log("pos_restaurant self", self)
        ...
        handles.on('dragend',    function(event,drag){ self.handle_dragend_handler(event,$(this),drag); });
			...
			$.each($('.table_name'), function(index , value){
				var a = $(value).text();
				a = a.trim();
				for (var i = 0; i < self.pos.attributes.orders.length; i++) {
					if(self.table.name != ""){
						if(self.pos.attributes.orders.models[i].table.name == a){		
						var total = 0;
						console.log("IN", self);
						for (var j = 0; j < self.pos.attributes.orders.models[i].orderlines.length; j++) {									
							var price = self.pos.attributes.orders.models[i].orderlines.models[j].price;
							var qty = self.pos.attributes.orders.models[i].orderlines.models[j].quantity;
							total = total + (price * qty);
							//total = total + (total * 0.19)
						}
						var cashier = self.pos.attributes.orders.models[i].ms_info.created.user.name;
						if(total<=0){
							//$($('.table_id')[index]).text(self.pos.attributes.orders.models[i].table.name);
							$(value).text(self.pos.attributes.orders.models[i].table.name);
						}
						else{
							//$($('.table_id')[index]).text(cashier + "-" + total.toFixed(0));
							var newElem = document.createElement("BR");
							var finalvalue = "<br/><span>"+ self.pos.attributes.orders.models[i].table.name +"</span>"
							var finalcashier = "<br/><span>"+ cashier +"</span>"
							var finaltotal = "<br/><span>"+ total.toFixed(0) +"</span>"
							var htmlObject = $(finalcashier+finaltotal);	
							$(value).append(htmlObject);
						}
					}
				}
			}	
...
chrome.OrderSelectorWidget.include({
    ...
    renderElement: function(){
        ...
        if (this.pos.config.iface_floorplan) {
			...
			//setTimeout(function(){
			//console.log("self.orderwidget", self);
			//for (var i = 0; i < self.pos.attributes.orders.length; i++) {
				//if(self.pos.attributes.orders.models[i].table.name == self.pos.table.name){
					//var total = 0;
					//for (var j = 0; j < self.pos.attributes.orders.models[i].orderlines.length; j++) {
						//var price = self.pos.attributes.orders.models[i].orderlines.models[j].price;
						//var qty = self.pos.attributes.orders.models[i].orderlines.models[j].quantity;
						//total = total + (price * qty);
						//console.log("total Table :", total, self.pos.table.name);
					//}
					//$.each($('.table_name'), function(index , value){
						//var self1 = this;
						//var a = $(value).text();
						//var b = value.textContent || value.innerText;
						//a = a.trim();
						//b = b.trim();
						//console.log("each a :", a);
						//console.log("each value:", value);
						//console.log("each b", b);
						//if(a==self.pos.table.name){
							//console.log("each value:", value);	
							//$('.table_id')[index].innerHTML = total;
							//console.log("each self.pos.table.name:", self.pos.table.name);
						//}										
					//});
				//}
			//} 
